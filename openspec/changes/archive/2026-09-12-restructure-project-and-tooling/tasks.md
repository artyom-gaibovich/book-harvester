...
- [x] 6.1 Создать резервный тег/ветку текущего `master` перед переписыванием; проверить, что резервная ссылка существует (`git tag`/`git branch`)
- [x] 6.2 Получить у пользователя явное разрешение на переписывание опубликованной истории и `push --force` (требование git-rules); без разрешения — остановиться
- [x] 6.3 Squash истории в осмысленные коммиты с кодами `IPRD-XXX` (тулинг, реструктуризация кода, конфигурация, документация) по формату `{CODE}-NNN: описание` + тело; проверить `git log --oneline` — коммиты соответствуют формату заголовка
- [x] 6.4 После подтверждения — `git push --force`; проверить, что удалённый `master` соответствует локальному
